...
sbInsertSQL1.append("  JOIN disease_categories_t  dc ON rpc.dc_id = dc.dc_id");
...
sbInsertSQL2.append(",raai.amount_oblig ");
...
sbInsertSQL2.append("    AND rpc.code_percent_relevance > 0");
...
if(bRunByApplID){
	...
	sbSelect.append(",COALESCE(rcdc.appl_status_descrip,flare.appl_status_code) AS appl_status_descrip");
	...
	sbSelect.append(", '<a href=\"https://rcdc.era.nih.gov:443/rcdc/viewExportedProject.jsp?action=VIEW_PDF&applicationID='||COALESCE(rcdc.appl_id,flare.appl_id)||'\" target=\"_blank\" >PDF Link</a> ' AS pdf_link");
	sbSelect.append(", '<a href=\"https://rcdc.era.nih.gov:443/rcdc/viewExportedProject.jsp?action=VIEW_EXTRACTED_TEXT&applicationID='||COALESCE(rcdc.appl_id,flare.appl_id)||'\" target=\"_blank\" >Extracted Text Link</a> ' AS ext_link");
	...
	sbSelect.append(" ELSE 'BOTH'");
	sbSelect.append(" END AS data_source");
	sbSelect.append(", '<a href=\"https://rcdc.era.nih.gov:443/rcdc/viewExportedProject.jsp?action=VIEW_PDF&applicationID='||MAX(COALESCE(rcdc.appl_id,flare.appl_id))||'\" target=\"_blank\" >PDF Link</a> ' AS pdf_link");
	sbSelect.append(", '<a href=\"https://rcdc.era.nih.gov:443/rcdc/viewExportedProject.jsp?action=VIEW_EXTRACTED_TEXT&applicationID='||MAX(COALESCE(rcdc.appl_id,flare.appl_id))||'\" target=\"_blank\" >Extracted Text Link</a> ' AS ext_link");
	sbSelect.append(" FROM ").append(sTblName1).append(" rcdc");
	sbSelect.append(" FULL OUTER JOIN   ").append(sTblName2).append(" flare ON rcdc.fy=flare.fy AND rcdc.appl_id=flare.appl_id");
	if(source.equals("Both")){
		sbSelect.append(" where flare.fy IS NOT NULL and rcdc.fy IS NOT NULL ");
